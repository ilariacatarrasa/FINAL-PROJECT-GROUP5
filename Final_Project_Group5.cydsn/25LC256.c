...
uint8_t EEPROM_readStatus() {
	...
	return SPI_Interface_25LC256_ReadByte(SPI_EEPROM_RDSR);
...
void EEPROM_writeEnable() {
	...
	SPI_Interface_25LC256_tradeByte(SPI_EEPROM_WREN);
...
uint8_t EEPROM_readByte(uint16_t addr) {
	...
	SPI_Interface_25LC256_Multi_RW(dataTX, 3, &dataRX, 1);
	return dataRX;
...
void EEPROM_writeByte(uint16_t addr, uint8_t dataByte) {
	...
	SPI_Interface_25LC256_Multi_RW(dataTX, 4, &temp, 0);
...
void EEPROM_readPage(uint16_t addr, uint8_t* dataRX, uint8_t nBytes) {
	...
	/* Read the nBytes */
	SPI_Interface_25LC256_Multi_RW(dataTX, 3, dataRX, nBytes);
...
void EEPROM_writePage(uint16_t addr, uint8_t* data, uint8_t nBytes) {
    ...
    */
	...
	SPI_Interface_25LC256_Multi_RW(dataTX, 3+nBytes, &temp, 0);
